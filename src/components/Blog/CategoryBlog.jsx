...
function CategoryBlog(par) {
  ...
  useEffect(() => {
    ...
    API.get(data, `user/count-blog-category/${params.id}`).then((res) => {
      if (res) {
    ...
    API.get(data, `user/get-category-blog/${params.id}/${page}`).then((res) => {
      if (res) {
        setBlog(res.result);
      } else {
        console.log("error");
  ...
  const catAct = (ids) => {
    ...
    const data = { data: "data" };
    API.get(data, `user/get-category-blog/${ids}/${page}`).then((res) => {
      if (res) {
        setBlog(res.result);
      } else {
        console.log("error");
          ...
          <div className="row blog-page">
            <div className="col-lg-8 col-md-12 col-sm-12 col-xs-12">
                ...
                {blog ? (
                  blog.map((item, index) => {
                    return (
                      <article
                        kye={index}
                        className="hentry post post-standard has-post-thumbnail"
                      >
                        <div
                        ...
                        >
                          ...
                          <a className="link-image js-zoom-image">
                            <svg className="utouch-icon utouch-icon-zoom-increasing-button-outline">
                              <use xlinkHref="#utouch-icon-zoom-increasing-button-outline"></use>
                            </svg>
                          </a>
                          <a className="link-post">
                            <svg className="utouch-icon utouch-icon-link-chain">
                              <use xlinkHref="#utouch-icon-link-chain"></use>
                            </svg>
                          </a>
                        ...
                        <div className="post__content">
                          ...
                          <div className="post__content-info">
                            <a className="h5 post__title entry-title">
                              {parse(item.title)}
                            </a>
                            <p className="post__text">
                              {parse(item.description.slice(0, 500) + "...")}
                              <a
              ...
              {blog.length > 0 ? (
                ...
                <img
                  class="not-found"
                  src={process.env.PUBLIC_URL + "/img/found-error.png"}
                  alt="404-error"
                />
              )}
            </div>
